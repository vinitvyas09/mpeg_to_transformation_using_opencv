...
int main( int argc, char **argv)
{
	...
	pthread_attr_init(&mysched_attr);
	pthread_attr_setinheritsched(&mysched_attr, PTHREAD_EXPLICIT_SCHED);
	pthread_attr_setschedpolicy(&mysched_attr, SCHED_FIFO);

	max_prio = sched_get_priority_max(SCHED_FIFO);
	my_param.sched_priority = max_prio;
	rc=sched_setscheduler(getpid(), SCHED_FIFO, &my_param);
	...
	printf("FIFO max prio = %d\n", max_prio);
	pthread_attr_getscope(&mysched_attr, &scope);
	pthread_attr_setschedparam(&mysched_attr, &my_param);
